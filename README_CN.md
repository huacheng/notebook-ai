...
**UI 布局（分屏模式 — 文件 + Notebook）：**

```
┌──────────────── 工具栏（模型选择、连接状态）─────────────────┐
├─ 侧边栏 ─┬─ 文件标签 ───┬─ Notebook 标签 ─ Git ─┬─ 右面板 ─┤
│ 项目     │  ┌────────┐   │  ┌─ 状态栏 ────────┐   │ 交付物  │
│ 文件     │  │FileView│   │  │  Cell 列表       │   │ 文件区  │
│ ── 线 ── │  │        │   │  │  ...流式输出...  │   │         │
│ Library  │  │        │   │  └─ 输入栏 ────────┘   │         │
│          │  └────────┘   │                         │         │
└──────────┴───────────────┴─────────────────────────┴─────────┘
...
面板支持拖拽调整大小。分屏模式下 Tab 栏拆分为左侧（文件 Tab）和右侧（Notebook + Git Tab）两组，与下方面板对齐。Git Tab 始终固定在右侧最后。
