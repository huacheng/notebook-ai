...
7. **Library search**: invoke `/task-ai:library search "<keywords>"` with verification-relevant keywords (testing frameworks, tools, domain standards). Library search handles index reading, scoring, and ranked results — read high-scoring matches for domain verification guidance. **Best Practice**: prefer `library search` over direct file reads for multi-factor scoring, graph recommendations, and token budget control
