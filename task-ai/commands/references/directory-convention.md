...
│   │       ├── .target.md             # 需求目标（人工编写）
│   │       ├── .convergence-baseline.md # 加权 R# 收敛评分基线（check 写入，ROLLBACK 判据）
...
- `.convergence-baseline.md` records the weighted R# convergence scores per requirement, used by `check` to compute convergence trend and trigger ROLLBACK when score does not improve
