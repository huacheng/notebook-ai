...
Where `wᵢ` is the weight of requirement i (default 1.0) and `cᵢ` is its completion score (0.0–1.0). The score is recorded in `.convergence-baseline.md` after each `check post-exec`, and the aggregate convergence value is stored in `stage.history[].convergence` upon stage completion.
...
|-----------|---------|--------|
| convergence > previous | ACCEPT | Proceed to merge → evolving |
| convergence ≈ previous (within tolerance) | NEEDS_FIX | Retry execution with guidance |
