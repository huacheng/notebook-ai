...
# F23: Target is <project>/.deliverables/<notebook>/
if '.deliverables/$NOTEBOOK' in merge_sh or '.deliverables/"$NOTEBOOK"' in merge_sh or "DELIVERABLES_TARGET" in merge_sh:
    print("PASS F23: merge.sh writes to <project>/.deliverables/<notebook>/ (main)")
else:
    print("FAIL F23: merge.sh missing project-level deliverables target path")
    FAIL += 1
...
if ".deliverables" in merge_skill and ".deliverables/<notebook>" in merge_skill:
    print("PASS F25: SKILL.md documents source→target path remapping")
else:
    print("FAIL F25: SKILL.md missing .deliverables → .deliverables/<notebook> path mapping")
    FAIL += 1
