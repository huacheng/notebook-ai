...
Checks:
  ...
  2. cancel State Transitions contains stage-done row
  3. report State Transitions contains stage-done row; Prerequisites mentions stage-done
  4. plan State Transitions contains stage-done REJECT row
  5. annotate State Transitions contains stage-done REJECT row
...
missing = {'review', 're-planning', 'blocked', 'complete', 'cancelled'} - target_statuses
if not missing:
    emit_pass('target: State Transitions has review/re-planning/blocked/complete/cancelled rows')
else:
    emit_fail(f'target: State Transitions missing rows for: {", ".join(sorted(missing))}')

# --- Test 2: cancel State Transitions has stage-done ---
...
if 'stage-done' in cancel_statuses:
    emit_pass('cancel: State Transitions has stage-done row')
else:
    emit_fail('cancel: State Transitions missing stage-done row')

# --- Test 3: report State Transitions has stage-done; Prerequisites mentions stage-done ---
...
if 'stage-done' in report_statuses:
    emit_pass('report: State Transitions has stage-done row')
else:
    emit_fail('report: State Transitions missing stage-done row')

if 'stage-done' in report_prereqs:
    emit_pass('report: Prerequisites mentions stage-done')
else:
    emit_fail('report: Prerequisites missing stage-done mention')

# --- Test 4: plan State Transitions has stage-done REJECT ---
...
plan_stage_done = [r for r in plan_rows if r.get('Current Status', '').strip('` ') == 'stage-done']

if plan_stage_done and 'REJECT' in plan_stage_done[0].get('After Plan', ''):
    emit_pass('plan: State Transitions has stage-done REJECT row')
else:
