...
describe('AutoStatusBar helpers', () => {
  it('exports PHASES and PHASE_MAP with correct mapping', async () => {
    const { PHASES, PHASE_MAP } = await import('../components/AutoStatusBar');
  ...
  it('getPhaseIndex returns correct index for each phase', async () => {
    const { getPhaseIndex } = await import('../components/AutoStatusBar');
  ...
  it('D_LABELS has entries for all six dimensions', async () => {
    const { D_LABELS } = await import('../components/AutoStatusBar');
