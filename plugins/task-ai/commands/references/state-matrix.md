...
| `executing` | =`executing` | →`re-planning` | →`re-planning` | ⊘ | CONT=`executing` / NEEDS_FIX=`executing` / REPLAN→`re-planning` / BLOCKED→`blocked` | ACCEPT=`executing` (auto→`evolving`) / NEEDS_FIX=`executing` / REPLAN→`re-planning` / ROLLBACK→`evolving` | =`executing` (NEEDS_FIX fix) / →`blocked` (dependency) | =`executing` (deliverables copy only) | — | →`cancelled` | — |
...
- `merge` requires ACCEPT verdict in `.analysis/` (pure file copy, no status change; auto handles status → `evolving` after check ACCEPT)
...
- **No inter-stage merge by auto**: In auto mode, deliverables accumulate on the task branch across stages. User can manually call `/task-ai:merge` at any time to copy current deliverables to main. `--satisfy` does not trigger merge — it only marks the task as satisfied
