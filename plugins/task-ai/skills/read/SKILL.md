...
6. **Write to library** using `library write`:
   ```bash
   /task-ai:library write ".memory/.references/<topic>.md" \
     --content-file <sanitized-content-file> \
     --notebook <notebook-name>
   ```
   The `library write` command handles the full 8-step protocol internally (locking, atomic write, changelog, index, relations, git commit).
7. **Rebuild** (optional): Invoke `/task-ai:library maintain --rebuild-index` if index consistency is needed.
