...
TARGET_DIR="$PROJECT_DIR/.worktrees/task-$NOTEBOOK_NAME"
...
if [[ ! -d "$PROJECT_DIR/.git" ]]; then
    echo "[ERROR] Project '$PROJECT_NAME' is not a git repository (no .git directory)" >&2
    exit 1
fi
...
if git -C "$PROJECT_DIR" branch --list "$BRANCH_NAME" | grep -qF "$BRANCH_NAME"; then
...
GIT_STATUS_OUTPUT=$(git -C "$PROJECT_DIR" status --porcelain 2>/dev/null) || {
...
cleanup() {
    ...
    if [[ -n "$CLEANUP_WORKTREE" && -d "$CLEANUP_WORKTREE" ]]; then
        git -C "$PROJECT_DIR" worktree remove --force "$CLEANUP_WORKTREE" 2>/dev/null || true
    fi
    if [[ -n "$CLEANUP_BRANCH" ]]; then
        ...
        git -C "$PROJECT_DIR" branch -D "$CLEANUP_BRANCH" 2>/dev/null || true
    fi
}
...
git -C "$PROJECT_DIR" branch "$BRANCH_NAME" || { echo "[ERROR] Failed to create branch $BRANCH_NAME" >&2; exit 1; }
CLEANUP_BRANCH="$BRANCH_NAME"

if [[ $USE_WORKTREE -eq 1 ]]; then
    # Worktree mode: create worktree at project/.worktrees/task-<notebook>
    WORKTREE_PATH="$PROJECT_DIR/.worktrees/task-$NOTEBOOK_NAME"
    git -C "$PROJECT_DIR" worktree add "$WORKTREE_PATH" "$BRANCH_NAME" || { echo "[ERROR] Failed to create worktree" >&2; exit 1; }
    CLEANUP_WORKTREE="$WORKTREE_PATH"
    WORKING_DIR="$WORKTREE_PATH/.working"
    CLEANUP_DIR="$WORKTREE_PATH"
else
    # Non-worktree mode: checkout branch in project directory
    git -C "$PROJECT_DIR" checkout "$BRANCH_NAME" || { echo "[ERROR] Failed to checkout $BRANCH_NAME" >&2; exit 1; }
    WORKING_DIR="$TARGET_DIR/.working"
    CLEANUP_DIR="$TARGET_DIR"
fi

# 4b. Ensure project .gitignore has task-ai entries (idempotent)
# In worktree mode, edit .gitignore inside the worktree; otherwise in project root
if [[ $USE_WORKTREE -eq 1 ]]; then
  _ROOT_GI="$WORKTREE_PATH/.gitignore"
else
  _ROOT_GI="$PROJECT_DIR/.gitignore"
fi
# Gitignore entries for task-ai (tracked notebooks, ignored temp files)
_GI_ENTRIES=(
  "**/.working/.auto-stop"
  "**/.working/.lock"
  "**/.working/.lock.stale.*"
...
# Git operations target worktree or project directory
if [[ $USE_WORKTREE -eq 1 ]]; then
