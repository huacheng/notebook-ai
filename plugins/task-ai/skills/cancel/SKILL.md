...
3. **Stop auto** if running:
   ...
   - Delete `.auto-stop` file if it exists
   - Handle `.lock` — first read lock content and verify the holder:
...
- **Concurrency**: Cancel acquires `.lock` before modifying files and releases on completion (see Concurrency Protection in `commands/task-ai.md`). Lock cleanup for the auto session (step 3) is separate from cancel's own lock acquisition (step 4)
